...
public class MonumentLocator {
    ...
    public static String getMonumentStreet(String longitude, String latitude){
        HttpResponse<String> response = getMonumentLocationInfo(longitude, latitude);
        try {
            ObjectMapper mapper = new ObjectMapper();
            JsonNode rootNode = mapper.readTree(response.body());
            return rootNode.get("address").get("road").asText();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static HashMap<String,String> getCoordinates(String monumentName){
        ...
        try {
            String monumentoFormateado = monumentName.replace(" ","%20");
